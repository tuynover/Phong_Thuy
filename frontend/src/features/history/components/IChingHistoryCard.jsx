...
export default function IChingHistoryCard({
    record,
    onView,
    onPreload,
    onTogglePublic,
    ...
    return (
        <div 
            ...
            onMouseEnter={() => onPreload('iching', record._id)}
            onTouchStart={() => onPreload('iching', record._id)}
        ...
        >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                <div className="space-y-1.5 flex-1 min-w-0">
                    <h3 className="font-bold text-base sm:text-lg text-amber-900 break-words">
                        {record.primaryHexagram.name} {record.transformedHexagram?.name ? `→ ${record.transformedHexagram.name}` : ''}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                        ...
                        <button
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                onOpenTagModal({ type: 'iching', record });
                            }}
                ...
                <div className="flex items-center gap-2 self-end sm:self-start shrink-0" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1.5 mr-1">
                        ...
                        <button
                            type="button"
                            onClick={() => onTogglePublic('iching', record._id, record.isPublic)}
                    ...
                    {record.isPublic && (
                        <button 
                            onClick={() => onCopyLink('iching', record._id)} 
                    ...
                    <button 
                        onClick={() => onTogglePin('iching', record._id)} 
                    ...
                    <button 
                        onClick={() => onOpenTagModal({ type: 'iching', record })} 
                        className="p-1.5 rounded-xl hover:bg-amber-50 text-amber-700 hover:text-amber-850 transition-colors cursor-pointer"
                    ...
                    <button 
                        onClick={() => onDelete('iching', record._id)} 
            ...
            <div onClick={(e) => e.stopPropagation()} className="mt-4 pt-4 border-t border-amber-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-default">
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    {renderStars(record.rating, (rating) => onRate('iching', record._id, rating, document.getElementById(`feedback-hex-${record._id}`)?.value || record.feedback))}
                </div>
                    ...
                    <button 
                        onClick={() => {
                            ...
                            if (val !== record.feedback || !record.rating) {
                                onRate('iching', record._id, record.rating, val);
                            }
                        }}
