...
export default function MarriageHistoryCard({
    record,
    onView,
    onPreload,
    onTogglePublic,
    ...
    return (
        <div 
            ...
            onMouseEnter={() => onPreload('marriage', record._id)}
            onTouchStart={() => onPreload('marriage', record._id)}
        ...
        >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                <div className="space-y-1.5 flex-1 min-w-0">
                    <h3 className="font-bold text-base sm:text-lg text-rose-900 break-words">
                        Hợp Hôn: Nam ({record.inputInfo?.male?.date || ''}) & Nữ ({record.inputInfo?.female?.date || ''})
                    </h3>
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                        ...
                        <button
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                onOpenTagModal({ type: 'marriage', record });
                            }}
                ...
                <div className="flex items-center gap-2 self-end sm:self-start shrink-0" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1.5 mr-1">
                        ...
                        <button
                            type="button"
                            onClick={() => onTogglePublic('marriage', record._id, record.isPublic)}
                    ...
                    {record.isPublic && (
                        <button 
                            onClick={() => onCopyLink('marriage', record._id)} 
                    ...
                    <button 
                        onClick={() => onTogglePin('marriage', record._id)} 
                    ...
                    <button 
                        onClick={() => onOpenTagModal({ type: 'marriage', record })} 
                        className="p-1.5 rounded-xl hover:bg-rose-50 text-rose-800 hover:text-rose-900 transition-colors cursor-pointer"
                    ...
                    <button 
                        onClick={() => onDelete('marriage', record._id)} 
            ...
            <div onClick={(e) => e.stopPropagation()} className="mt-4 pt-4 border-t border-rose-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-default">
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    {renderStars(record.rating, (rating) => onRate('marriage', record._id, rating, document.getElementById(`feedback-marr-${record._id}`)?.value || record.feedback))}
                </div>
                    ...
                    <button 
                        onClick={() => {
                            ...
                            if (val !== record.feedback || !record.rating) {
                                onRate('marriage', record._id, record.rating, val);
                            }
                        }}
